...
import { Trophy, Factory, Truck, CreditCard, CheckCircle, Zap } from "lucide-react";
...
const features = [
  ...
  { icon: <CheckCircle size={20} />, title: "Quality Assurance", description: "Every lot checked before dispatch. Mill-certified grades. No quality surprises — ever.", color: "#6B9E78" },
  { icon: <Zap size={20} />, title: "Responsive Team", description: "Young, tech-enabled management. Fast response on WhatsApp, email, and phone.", color: "#5B8DB8" },
];

export default function WhyUs() {
  return (
    <section id="why-us" aria-labelledby="whyus-heading" className="section-padding" style={{ background: "var(--black)" }}>
      <div className="section-container">
        ...
        >
          ...
          <p className="text-base leading-relaxed" style={{ color: "var(--text-secondary)" }}>
            Built on relationships, backed by systems. The reliability of a 15-year business with the speed of a modern operation.
          </p>
        </motion.div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {features.map((feat, i) => (
            <FeatureBlock key={feat.title} icon={feat.icon} title={feat.title} description={feat.description} index={i} accentColor={feat.color} />
          ))}
        </div>
      </div>
    </section>
